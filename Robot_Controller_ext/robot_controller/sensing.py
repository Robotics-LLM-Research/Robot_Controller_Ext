...
class SensorSuite:
    ...
    def attach(self):
        ...
        if self._cam_path is not None:
            ...
            if not cam_prim or not cam_prim.IsValid():
                log(f"[SENSE] no camera prim found for {FRONT_CAM_PRIM} under body; camera not attached", 3)
                self._cam_path = None
            elif not cam_prim.IsA(UsdGeom.Camera):
                log(
                    f"[SENSE] {self._cam_path} is not a UsdGeom.Camera; camera not attached",
                    3,
                )
                self._cam_path = None
            else:
    ...
    def _try_init_camera(self):
        ...
        if self._camera_init_attempts <= self._camera_warmup_steps:
            return
